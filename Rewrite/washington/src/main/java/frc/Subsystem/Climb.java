...
public class Climb extends AbstractSubsystem {
    ...
    public synchronized void manualClimb(){
      ...
      public synchronized void resetClimb(){
        if(magSensor.get() && intakeSolenoid.get() == Value.kReverse && Math.abs(midClimb.getSelectedSensorPosition() - 590000) > 1000)
          midClimb.set(ControlMode.Position, 590000);
        else if(!magSensor.get() && intakeSolenoid.get() == Value.kReverse && Math.abs(midClimb.getSelectedSensorPosition() - 590000) < 1000){
